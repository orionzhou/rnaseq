...
th = res$th %>%
    mutate(lab = str_c(SampleID, Treatment, Replicate, sep='_')) %>%
...
p2 = plot_pca(tm,th,pct.exp=.7, pca.center=T, pca.scale=F,
    var.shape='Treatment', var.col='Treatment', var.lab='clab', var.ellipse='Treatment',
...
p3 = plot_tsne(tm,th,pct.exp=.7,perp=2,iter=1200, seed=42,
    var.shape='Treatment', var.col='Treatment', var.lab='clab', var.ellipse='Treatment',
    legend.pos='top.right', legend.dir='v', pal.col='aaas')
