...
th2 = th %>% filter(SampleID %in% sids_keep) %>%
    mutate(Genotype=ifelse(SampleID=='SRR1837669', 'Oh603', Genotype)) %>%
    mutate(Genotype=ifelse(SampleID=='SRR1837983', 'CML218', Genotype)) %>%
    mutate(Genotype=ifelse(SampleID=='SRR1837710', 'T8', Genotype))
...
res = rnaseq_cpm(yid)
th = res$th; tm = res$tm; tl = res$tl; th_m = res$th_m; tm_m = res$tm_m
th = th %>% mutate(lab = sprintf("%s_%d", Genotype, Replicate))
...
ggsave(p1, filename = fo, width=8, height=27)
#}}}
...
p_tsne = ggplot(tp) +
    ...
    geom_point(aes(x=V1, y=V2, color=as.character(Replicate), shape=as.character(Replicate)), size=2) +
