...
p2 = plot_pca(tm,th,pct.exp=.7, pca.center=T, pca.scale=F,
    var.shape='Genotype', var.col='Tissue', var.lab='lab',
    legend.pos='none', legend.dir='v', pal.col='aaas')
ggsave(file.path(dirw, '11.pca.pdf'), p2, width=6, height=6)

p3 = plot_tsne(tm,th,pct.exp=.7,perp=3,iter=1500, seed=2,
    var.shape='Genotype', var.col='Tissue', var.lab='lab',
    legend.pos='none', legend.dir='v', pal.col='aaas')
...
th2 = res$th
#th2 = res$th %>% filter(SampleID %in% sids_keep) %>% filter(SampleID != 'SRR1567900')
...
p3 = plot_tsne(tm,th,pct.exp=.7,perp=3,iter=1500, seed=2,
    var.shape='Genotype', var.col='Tissue', var.lab='lab',
    legend.pos='none', legend.dir='v', pal.col='aaas')
ggsave(file.path(dirw, '12.tsne.pdf'), p3, width=6, height=6)
#}}}
