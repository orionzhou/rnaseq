...
res = rnaseq_cpm(yid)
th = res$th; tm = res$tm; tl = res$tl; th_m = res$th_m; tm_m = res$tm_m
th = th %>% mutate(Treatment=sprintf("%03d", Treatment)) %>%
    mutate(lab = sprintf("%s_%s", Treatment, Replicate))
...
p_tsne = ggplot(tp) +
    ...
    geom_point(aes(x=V1, y=V2, color=Treatment, shape=as.character(Replicate)), size=2) +
    scale_x_continuous(name = 'tSNE-1') +
    scale_y_continuous(name = 'tSNE-2') +
    scale_shape_manual(values = c(15,0)) +
    scale_color_viridis_d(name = 'Timepoint',direction=-1) +
