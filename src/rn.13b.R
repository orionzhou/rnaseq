...
res = rnaseq_cpm(yid)
th = res$th; tm = res$tm; tl = res$tl; th_m = res$th_m; tm_m = res$tm_m
th = th %>% mutate(Etiolated = str_detect(Treatment,"^E")) %>%
...
p1 = p1 %<+%
    tp + geom_tiplab(aes(label=Treatment, color=Etiolated), size=2.5) +
    scale_color_aaas()
...
p_tsne = ggplot(tp) +
    ...
    geom_point(aes(x=V1, y=V2, color=Timepoint, shape=Etiolated), size=2) +
    scale_x_continuous(name = 'tSNE-1') +
    scale_y_continuous(name = 'tSNE-2') +
    scale_shape_manual(values = c(15,0)) +
    scale_color_viridis(name = 'Timepoint') +
