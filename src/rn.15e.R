...
res = rnaseq_cpm(yid)
th = res$th; tm = res$tm; tl = res$tl; th_m = res$th_m; tm_m = res$tm_m
th = th %>% mutate(Treatment = sprintf("%03d", as.integer(Treatment))) %>%
    mutate(lab = sprintf("%s_%s_%s", Genotype, Treatment, Replicate))
...
#    geom_text_repel(aes(x=V1,y=V2,label=Genotype), size=2.5) +
    geom_point(aes(x=V1, y=V2, shape=Genotype, color=Treatment), size=2) +
    scale_x_continuous(name = 'tSNE-1') +
    scale_y_continuous(name = 'tSNE-2') +
    scale_color_viridis_d(name = 'Timepoint') +
