...
th = res$th %>%
    ...
    mutate(lab = str_c(SampleID, Genotype, Tissue, Replicate, sep='_')) %>%
...
p1 = plot_hclust(tm,th,pct.exp=.7,cor.opt='pearson',var.col='Tissue',
    pal.col='viridis_d', expand.x=.3)
ggsave(file.path(dirw, '11.hclust.p.pdf'), p1, width=8, height=20)

p1 = plot_hclust(tm,th,pct.exp=.7,cor.opt='spearman',var.col='Tissue',
    pal.col='viridis_d', expand.x=.3)
ggsave(file.path(dirw, '11.hclust.s.pdf'), p1, width=8, height=20)
...
p1 = plot_hclust(tm,th,pct.exp=.7,cor.opt='pearson',var.col='Tissue',
    pal.col='viridis_d', expand.x=.3)
ggsave(file.path(dirw, '21.hclust.p.pdf'), p1, width=8, height=20)

p1 = plot_hclust(tm,th,pct.exp=.7,cor.opt='spearman',var.col='Tissue',
    pal.col='viridis_d', expand.x=.3)
ggsave(file.path(dirw, '21.hclust.s.pdf'), p1, width=8, height=20)
