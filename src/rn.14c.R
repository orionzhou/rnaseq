...
th = th %>% replace_na(list(Treatment='')) %>%
    mutate(Tissue=str_c(Tissue,Treatment,sep=''))
...
tp = th %>% mutate(taxa = SampleID) %>%
    mutate(lab = sprintf("%s",Tissue,Replicate)) %>%
...
ggsave(p1, filename = fo, width=3, height=5)
#}}}
