...
p1 = ggtree(tree, layout = 'rectangular') +
    scale_x_continuous(expand = expand_scale(0,.25)) +
...
x.max=max(tp$V1)
p_tsne = ggplot(tp) +
    geom_text_repel(aes(x=V1,y=V2,label=Replicate), size=2.5) +
    geom_point(aes(x=V1, y=V2, color=Genotype, shape=Treatment), size=2) +
